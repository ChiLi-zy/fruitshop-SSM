...
public class RetailerController extends BaseController{
	@Resource
	RetailerService retailerService; //在实现类暴露 引用只引用接口
	//跳转至列表页面
	...
	public String list(Model model,Retailer retailer,String startTime,String endTime){
		...
		System.out.println("retailer-map------------"+map);
		...
		System.out.println("retailerList---------"+retailerList);
		

		model.addAttribute("telphone", retailer.getTelphone());
		model.addAttribute("status", retailer.getStatus());
	...
	private Map<String, Object> retailerToMap(Retailer retailer) {
		...
		map.put("creatTime", checkStringIsEmpty(retailer.getCreateTime()));
	...
	public @ResponseBody Retailer editRetailer(@RequestBody String json){//@RequestBody是获取json的信息
		String id=JSONObject.parseObject(json).getString("id");//可以用FastJson
		System.out.println("id--------"+id);
		return retailerService.get(id);
	}
	@RequestMapping("/retailer/edit.action")
	public String edit(Model model,Retailer retailer){
		System.out.println("retailerEdit--------"+retailer);
	...
	public String add(Model model,Retailer retailer){
		...
		System.out.println("add-retailer--------"+retailer);
		retailerService.insert(retailer);
		//构建新的列表查询条件，只需要status
		Retailer queryRetailer=new Retailer();
		queryRetailer.setStatus(-1);
		return list(model,queryRetailer,null,null);
	}
